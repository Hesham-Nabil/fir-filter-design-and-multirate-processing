...
ylabel('Magnitude (dB)', 'FontSize', 10);
grid on;
xlim([0 fs/6]); % Limit the x-axis to Nyquist frequency
...
[H_cheby1, w] = freqz(b_cheby1, a_cheby1, 1024, fs); % Calculate response with actual frequency (fs)
...
% Verify specs
pb_ripple_cheby1 = max(H_cheby1_dB(passband)) - min(H_cheby1_dB(passband)); % Calculate passband ripple
sb_atten_cheby1 = -max(H_cheby1_dB(stopband)); % Calculate stopband attenuation
...
figure;
plot(w, H_cheby1_dB, 'LineWidth', 1.5);
title('Chebyshev Type I Filter Frequency Response', 'FontSize', 12);
xlabel('Frequency (Hz)', 'FontSize', 10);
ylabel('Magnitude (dB)', 'FontSize', 10);
grid on;
xlim([0 fs/6]); % Limit the x-axis to Nyquist frequency
...
% Verify specs
[H_cheby2, w] = freqz(b_cheby2, a_cheby2, 1024);
